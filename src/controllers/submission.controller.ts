...
function decodeIfBase64(
  ...
  try {
    ...
    // strict check: normalize padding before compare
    const norm = (s: string) => s.replace(/=+$/, "");
    if (norm(buf.toString("base64")) !== norm(value))
...
export const createSubmission = async (req: Request, res: Response) => {
  try {
    ...
    const isBase64Encoded =
      req.query.base64_encoded === "true" || body.base64_encoded;
    ...
    const decodedExpectedOutput = decodeIfBase64(
    ...
    if (language.is_archived) {
      return res.status(422).json({
        language_id: [
          `language with id ${body.language_id} is archived and cannot be used anymore`,
        ],
      });
    }
    ...
    const inQueueStatus = await prisma.status.upsert({
      where: { name: "In Queue" },
      update: {},
      create: { name: "In Queue" },
    });
    ...
    await inngest.send({
      ...
      data: {
        ...
        cpuTimeLimit: body.cpu_time_limit ?? 2,

        memoryLimit: body.memory_limit
          ? Math.ceil(body.memory_limit / 1024)
          : 256,
        wallTimeLimit: body.wall_time_limit ?? 20,
      },
    });
    ...
    if (error.message?.includes("Base64")) {
      return res.status(422).json({
        error:
          "Invalid Base64 encoding in source_code, stdin, expected_output, or additional_files",
      });
    }
